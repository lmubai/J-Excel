...
/**
 * User: Hyberbin Date: 13-12-4 Time: 下午2:15
 * 
 * @param <T>
 */
public class ValidateExcelService<T extends BaseExcelVo> extends BaseExcelService {
	...
	protected ValidateExcelService(Class<T> voClass, Sheet sheet, String[] fields, DicCodePool dicCodePool) throws Exception {
		this(voClass, sheet, fields, dicCodePool, new DataBean(voClass, voClass.getAnnotation(ExcelVoConfig.class).inputFactory(), voClass.getAnnotation(ExcelVoConfig.class).outputFactory(), voClass
				.getAnnotation(ExcelVoConfig.class).validateClass()));
	}
