...
/**
 * 默认： 第一行为标题 第二行为隐藏的字段名 第三行为表头 从第四行开始为导入数据 第四行从第0列开始必须连续，否则视为后面的无效 User:
 * Hyberbin Date: 13-12-3 Time: 下午4:01
 * 
 * @param <T>
 */
public class ImportExcelService<T extends BaseExcelVo> extends BaseExcelService {
	 ...
	 */
	private void setColumnGroupField(FieldBean fieldBean, BaseExcelVo excelVo, Row sheetRow, int row, int index, DataBean dataBean, GroupConfig groupConfig) throws AdapterException,
			ColumnErrorException {
